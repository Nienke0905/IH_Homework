...
let board = [
      ...
      case "N":
        if (rover.y > 0) {
          if (board[rover.y - 1][rover.x] === "") {
            ...
            console.log(
              `${rover.name} ran into an obstacle or other Rover at x: ${rover.x} y: ${rover.y - 1}.`
            );
          }
        } else {
      ...
      case "E":
        if (rover.x < 9) {
          if (board[rover.y][rover.x + 1] === "") {
            ...
            console.log(
              `${rover.name} ran into an obstacle or other Rover at x: ${rover.x + 1} y: ${rover.y}.`
            );
          }
        } else {
          console.log(`${rover.name} would be off Mars!`);
  ...
  function drivingRover(ro1, ro2) {
    ...
    for (
      let i = 0;
      ro1.count < ro1.commands.length && ro2.count < ro2.commands.length; i++) {
